...
const niz = ['Jedan', 'Dva', 'Tri', 'Četiri', 'Pet']
...
/* 6a - Napravite funkciju pomoću koje ćemo zadatak broj 5 uraditi
 korišćenjem DOM metoda i moći da kreiramo tabelu drugih dimenzija.
  Tako kreiranu tabelu napišite na kraj body-ja */
...
/* 8 - Napravite funkciju koja će za prosleđeni Date objekat da nam vrati datum u sledećem formatu:
 "2020-02-24 17:38:45" // dan, mesec, godina, sat, minut, sekund .
  U konzolu upišite rezultat za Date objekat kreiran od vašeg rođendana */
...
function randomBroj1 (x, y) {
  return Math.floor(Math.random() * (y - x)) + x
}

// 11 - zamenite sva pojavljivanja jednog stringa u drugom
function replaceAll (search, replace, subject) {
  while (subject.includes(search)) {
    subject = subject.replace(search, replace)
  }
  return subject
}

/* 12 - Vratite niz od 20 brojeva od 0 do 100 bez ponavljanja, greške napisane ispod moraju biti vraćene
  na ovaj način i u ovom formatu, posle nje napišite funkciju koja vraća random x elemenata objekta
  koji joj prosledimo, ta funkcija može da koristi funkciju koju smo napravili za vraćanje random
  niza jedinstvenih brojeva */
...
function randomObjectElements (obj, count) {
  ...
  // let tmpArray = Object.keys(obj), returnObject = {}
  const randomElements = randomNumbers(0, tmpArray.length - 1, count)
  for (const randomArrayElement in randomElements) {
    const objectKey = tmpArray[randomElements[randomArrayElement]]
...
function css (element, cssRules) {
  for (const stil in cssRules) {
    const osobineNiz = stil.split('-')
    let osobina = ''
    for (let i = 0; i < osobineNiz.length; i++) {
      if (i === 0) {
...
document.body.addEventListener('keydown', function (e) {
  const osobineDogadjaja = [e.key, e.keyCode, e.altKey, e.shiftKey, e.ctrlKey]
...
const randomKey = keysArray[Math.floor(keysArray.length * Math.random())]
// return obj[randomKey] treba li ovo biti unutar funckije?
...
function meni (poruka) {
  ...
  if (odgovor === 0) {
    ...
    const boje = ['red', 'green', 'blue', 'yellow', 'olive']
    const randomBoja = boje[Math.floor(Math.random() * boje.length)]
...
const niz5 = 'a,b,c,d,e,f,g,h'.split(',').join(':').split(':')[4]
...
document.getElementById('all').addEventListener('change', function () {
  const elementi = document.querySelectorAll('input[id^=check]')
...
function allChecked (selektor) {
  const elementi = document.querySelectorAll(selektor)
  let checked = 0
  for (let i = 0; i < elementi.length; i++) {
...
const tajmer = setInterval(function () {
  const trenutnaVrednost = parseInt(document.getElementById('tajmerOutput').innerHTML, 10)
...
function animacija () {
  ...
  let smer = 'dole'
  let smer2 = 'desno'
  const speedHorizontal = 10
  const speedVertical = 10
